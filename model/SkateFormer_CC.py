...
class SkateFormerBlock(nn.Module):
    def __init__(self, in_channels, num_points=50, kernel_size=7, num_heads=32,
        ...
        # 可学习的 channel 分配参数 (C, 3)
        self.channel_score = nn.Parameter(torch.randn(in_channels*2, 3))
    ...
    def forward(self, input):
        ...
        f = self.mapping(self.norm_1(input))  # [N, T, V, 2C]
        f = f.permute(0, 3, 1, 2).contiguous()  # [N, 2C, T, V]

        # -------------------------------------------------
        # 关键改动：根据可学习 channel_score 对通道进行分配
        # -------------------------------------------------
        # f_conv, f_attn = torch.split(f, [C // 2, 3 * C // 2], dim=1)
        score = F.softmax(self.channel_score, dim=1)  # shape = (C, 3)
        C_g = C // 4  # 分给 GConv
        C_t = C // 4  # 分给 TConv
        C_m = 3 * C // 2  # 剩下的给 SkateMSA
        gconv_idx = torch.argsort(score[:, 0], descending=True)[:C_g]
        tconv_idx = torch.argsort(score[:, 1], descending=True)[:C_t]
        msa_idx = torch.argsort(score[:, 2], descending=True)[:C_m]
        f_gconv = torch.index_select(f, dim=1, index=gconv_idx)  # [N, C_g, T, V]
        f_tconv = torch.index_select(f, dim=1, index=tconv_idx)  # [N, C_t, T, V]
        f_msa = torch.index_select(f, dim=1, index=msa_idx)  # [N, C_m, T, V]

        y = []

        # G-Conv
        y_gconv = []
        split_f_gconv = torch.chunk(f_gconv, self.gconv.shape[0], dim=1)
        ...
        # T-Conv
        y.append(self.tconv(f_tconv))

        # Skate-MSA
        split_f_attn = torch.chunk(f_msa, len(self.partition_function), dim=1)
        for i in range(len(self.partition_function)):
