...
class SkateFormer(nn.Module):
    ...
    def forward(self, input, index_t):
        ...
        if self.index_t:
            ...
            # 使用 OATPE 生成位置编码
            te = torch.zeros(B, T, self.embed_dim).to(output.device)  # B, T, embed_dim
            # OATPE 的生成方式
            orthogonal_matrix = torch.randn(self.embed_dim, self.embed_dim).to(output.device)
            orthogonal_matrix, _ = torch.linalg.qr(orthogonal_matrix, mode='reduced')  # 使用 QR 分解获取正交矩阵
            # 调整时间步生成方式
            time_indices = torch.arange(0, T, dtype=torch.float).to(output.device)  # 时间步 (T,)
            position_encoding = torch.matmul(time_indices.unsqueeze(-1),
                                             orthogonal_matrix[:1, :])  # 广播计算 (T, embed_dim)
            # 将生成的 OATPE 位置编码分配到 te
            te[:, :, :] = position_encoding.unsqueeze(0)  # 扩展到 (B, T, embed_dim)
            # 融合位置编码和 joint_person_embedding
            output = output + torch.einsum('b t c, c v -> b c t v', te, self.joint_person_embedding)  # C=192
