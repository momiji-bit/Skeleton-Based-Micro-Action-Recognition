...
class SELayer(nn.Module):
    def __init__(self, channel, reduction=16, top_k=8):
        """
        channel: 输入张量的通道数
        reduction: 缩放比，默认为16，可根据需求做调整
        """
        super(SELayer, self).__init__()
        self.top_k = top_k
        self.avg_pool = nn.AdaptiveAvgPool2d(1)  # 在 H, W 维度进行全局平均池化
    ...
    def forward(self, x):
        ...
        # Excitation：全连接层 + 激活
        y = self.fc(w).view(b, c, 1, 1)  # [B, C, 1, 1]
        # Recalibration：通道注意力加权
        x_recalibrated = x * y  # [B, C, H, W]

        # 获取每个批次的 top-k 通道
        # y.squeeze() 将 y 从 [B, C, 1, 1] 转换为 [B, C]
        y_squeezed = y.view(b, c)  # [B, C]
        topk_weights, topk_indices = torch.topk(y_squeezed, self.top_k, dim=1)  # [B, top_k]

        return x_recalibrated, topk_indices, topk_weights
...
class SkateFormerBlock(nn.Module):
    def __init__(self, in_channels, num_points=50, kernel_size=7, num_heads=32,
                 ...
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm, se_reduction=4):
        ...
        se = []
        for i in range(3):
            se.append(SELayer(channel=in_channels*2, reduction=se_reduction))
    ...
    def forward(self, input):
        ...
        G_se_out, w = self.se[0](f)  # [B, C, T, V]，每个sample都有自己的通道权重
        channel_importance = G_se_out.mean(dim=(0, 2, 3))  # 对 (B, T, V) 维度做平均，得到全局通道权重
        sorted_indices = torch.argsort(channel_importance, descending=True)  # [C]
        gconv_idx = sorted_indices[:C1]
        print(gconv_idx)
        gconv_input = f[:, gconv_idx, :, :]

        # 下一个 C2 给 T-Conv
        T_se_out = self.se[1](f)  # [B, C, T, V]，每个sample都有自己的通道权重
        channel_importance = T_se_out.mean(dim=(0, 2, 3))  # 对 (B, T, V) 维度做平均，得到全局通道权重
        sorted_indices = torch.argsort(channel_importance, descending=True)  # [C]
        tconv_idx = sorted_indices[C1:C1 + C2]
        tconv_input = f[:, tconv_idx, :, :]


        # 剩余的给 MSA
        MSA_se_out = self.se[1](f)  # [B, C, T, V]，每个sample都有自己的通道权重
        channel_importance = MSA_se_out.mean(dim=(0, 2, 3))  # 对 (B, T, V) 维度做平均，得到全局通道权重
        sorted_indices = torch.argsort(channel_importance, descending=True)  # [C]
        msa_idx = sorted_indices[C1 + C2:]
        msa_input = f[:, msa_idx, :, :]

        y = []

        # G-Conv
        split_f_gconv = torch.chunk(gconv_input, self.gconv.shape[0], dim=1)
        y_gconv = []
        for i in range(self.gconv.shape[0]):
            z = torch.einsum('n c t u, v u -> n c t v', split_f_gconv[i], self.gconv[i])
            y_gconv.append(z)
        y_gconv = torch.cat(y_gconv, dim=1)  # [B, C1, T, V]
        y.append(y_gconv)

        # T-Conv
        y_tconv = self.tconv(tconv_input)  # [B, C2, T, V]
        y.append(y_tconv)

        # Skate-MSA
        split_f_attn = torch.chunk(msa_input, len(self.partition_function), dim=1)
        for i in range(len(self.partition_function)):
            ...
            # attention
            y_msa = self.attention[i](input_partitioned)
            # reverse
            y_msa = self.reverse_function[i](y_msa, (T, V), self.partition_size[i])  # [B, C_msa, T_msa, V_msa]
            y.append(y_msa)
