...
class SkateFormerBlock(nn.Module):
    def __init__(self, in_channels, num_points=50, kernel_size=7, num_heads=32,
        ...
        # -------------- 下面为每个分支添加 SE 模块 -------------- #
        # G-Conv 输入通道 = in_channels // 4（因为先 split 到 C//2，再 chunk 2 块）
        self.se_gconv = SELayer(channel=in_channels // 4, reduction=se_reduction)

        # T-Conv 输入通道 = in_channels // 4
        self.se_tconv = SELayer(channel=in_channels // 4, reduction=se_reduction)

        # Skate-MSA 输入通道 = in_channels // (len(self.partition_function) * 2)
        # 需要为每个分支都来一个 SE
        self.se_attention = nn.ModuleList([
            SELayer(channel=in_channels * 2 * 3 // (len(self.partition_function) * 4), reduction=se_reduction)
            for _ in range(len(self.partition_function))
        ])
    ...
    def forward(self, input):
        ...
        # G-Conv
        # 先经过 SE
        gconv_input = self.se_gconv(split_f_conv[0])
        y_gconv = []
        split_f_gconv = torch.chunk(gconv_input, self.gconv.shape[0], dim=1)  # 依照头数再进一步拆分
        for i in range(self.gconv.shape[0]):
            z = torch.einsum('n c t u, v u -> n c t v', split_f_gconv[i], self.gconv[i])
            y_gconv.append(z)
        y_gconv = torch.cat(y_gconv, dim=1)  # [B, C//4, T, V]
        y.append(y_gconv)

        # T-Conv
        # 先经过 SE
        tconv_input = self.se_tconv(split_f_conv[1])
        # tconv_input shape = [B, C//4, T, V]
        y_tconv = self.tconv(tconv_input)  # [B, C//4, T, V] => [B, C//4, T, V]
        y.append(y_tconv)
        ...
        for i in range(len(self.partition_function)):
            # 先对输入做 SE
            attn_input = self.se_attention[i](split_f_attn[i])  # [B, C_msa, T, V]
            C = split_f_attn[i].shape[1]
            input_partitioned = self.partition_function[i](attn_input, self.partition_size[i])
