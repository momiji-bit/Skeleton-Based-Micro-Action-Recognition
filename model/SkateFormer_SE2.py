...
class SkateFormerBlock(nn.Module):
    def __init__(self, in_channels, num_points=50, kernel_size=7, num_heads=32,
                 ...
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm, se_reduction=16):
        ...
        # 先对整个输入做一次 SE
        self.se_all = SELayer(channel=in_channels*2, reduction=se_reduction)
    ...
    def forward(self, input):
        ...
        se_out = self.se_all(f)  # [B, C, T, V]，每个sample都有自己的通道权重
        channel_importance = se_out.mean(dim=(0, 2, 3))  # 对 (B, T, V) 维度做平均，得到全局通道权重
        ...
        # gconv_input, tconv_input, msa_input => shape 依旧是 [B, x, T, V]
