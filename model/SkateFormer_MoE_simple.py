...
class MOE(nn.Module):
    def __init__(self, in_features, hidden_features, act_layer, num_experts=4, drop=0., gating_hidden_dim=128):
        ...
        # 多层门控网络，这里使用两层 MLP
        self.gating_network = nn.Linear(in_features, num_experts)

    def forward(self, x):
        B, T, V, C = x.shape
        x_2d = x.view(B, T * V, C)  # (B, N, C)
        # gating_scores: (B, N, num_experts)
        gating_scores = self.gating_network(x_2d)
        top1_indices = torch.argmax(gating_scores, dim=-1)  # (B, N)
        output_2d = torch.zeros_like(x_2d)

        for e_id in range(self.num_experts):
            mask = (top1_indices == e_id)
            if mask.any():
                x_e = x_2d[mask]  # shape: [num_routed_tokens, C]
                y_e = self.experts[e_id](x_e)  # shape: [num_routed_tokens, C]
                output_2d[mask] = y_e

        output = output_2d.view(B, T, V, C)
        return output
