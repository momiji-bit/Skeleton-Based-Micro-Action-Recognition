...
class SkateFormerBlock(nn.Module):
    ...
    def forward(self, input):
        ...
        for i in range(len(self.partition_function)):
            C = split_f_attn[i].shape[1]  # 36
            input_partitioned = self.partition_function[i](split_f_attn[i], self.partition_size[i])  # B,C36,T64,V48
            input_partitioned = input_partitioned.view(-1, self.partition_size[i][0] * self.partition_size[i][1], C)
            att_out = self.attention[i](input_partitioned)
            reverse_out = self.reverse_function[i](att_out, (T, V), self.partition_size[i])
            y.append(self.alpha_attn[i] * reverse_out)
