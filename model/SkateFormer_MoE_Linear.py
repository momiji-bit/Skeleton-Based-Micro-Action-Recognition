...
class MOE(nn.Module):
    def __init__(self, in_features, hidden_features, act_layer, num_experts=3, drop=0., gating_hidden_dim=128):
        super(MOE, self).__init__()
        self.num_experts = num_experts
        self.experts = nn.ModuleList([
            Mlp(in_features, hidden_features, act_layer=act_layer, drop=drop)
            for _ in range(num_experts)
        ])
        # 多层门控网络，这里使用两层 MLP
        self.gating_network = nn.Sequential(
            nn.Linear(in_features, gating_hidden_dim),
            act_layer(),
    ...
    def forward(self, x):
        ...
        # Reshape 输入: (B, T, V, C) -> (B, N, C)
        x_2d = x.view(B, T * V, C)  # (B, N, C)
        # 计算门控分数
        gating_scores = self.gating_network(x_2d)  # (B, N, num_experts)
        gating_weights = F.softmax(gating_scores, dim=-1)  # Soft Gating

        # 初始化专家输出
        experts_output = []
        for e_id in range(self.num_experts):
            x_e = x_2d  # (B, N, C)
            y_e = self.experts[e_id](x_e)  # (B, N, C)
            experts_output.append(y_e.unsqueeze(-1))  # (B, N, C, 1)
        # 将所有专家的输出拼接在一起
        experts_output = torch.cat(experts_output, dim=-1)  # (B, N, C, num_experts)
        # 根据 gating 权重加权专家输出
        gating_weights = gating_weights.unsqueeze(2)  # (B, N, 1, num_experts)
        output = torch.sum(experts_output * gating_weights, dim=-1)  # (B, N, C)

        # reshape 回 (B, T, V, C)
        output = output.view(B, T, V, C)
        return output
...
class SkateFormerBlock(nn.Module):
    def __init__(self, in_channels, num_points=50, kernel_size=7, num_heads=32,
        ...
        self.moe = MOE(in_features=in_channels, hidden_features=int(mlp_ratio * in_channels), act_layer=act_layer,)
        self.proj_drop = nn.Dropout(p=drop)

        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()

        self.norm_2 = norm_layer(in_channels)
        self.mlp = Mlp(in_features=in_channels, hidden_features=int(mlp_ratio * in_channels), act_layer=act_layer,
                       drop=drop)
    ...
    def forward(self, input):
        ...
        # output = self.proj(torch.cat(y, dim=1).permute(0, 2, 3, 1).contiguous())  # Linear
        output = self.moe(torch.cat(y, dim=1).permute(0, 2, 3, 1).contiguous())  # MoE
